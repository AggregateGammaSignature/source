...
void run_scalar_tests2(void) {
	secp256k1_scalar s, o, a, b;
    /* Set 's' to a random scalar, with value 'snum'. */
    random_scalar_order_test(&s);

    secp256k1_scalar_set_int(&o, 1);
    secp256k1_scalar_set_int(&a, 1);
...
void pScalar(const secp256k1_scalar *r )
{
...
void test_gamma_sign_verify(void) {
    secp256k1_gej pubj;
    secp256k1_ge pub;
    secp256k1_scalar one;
    secp256k1_scalar msg, key;
    ...
    secp256k1_ge_set_gej(&pub, &pubj);
    getrec = secp256k1_rand_bits(1);
    random_gamma_sign(&sigr, &sigs, &key, &msg, getrec?&recid:NULL);
    if (getrec) {
        CHECK(recid >= 0 && recid < 4);
    }
    CHECK(secp256k1_gamma_sig_verify(&ctx->ecmult_ctx, &sigr, &sigs, &pub, &msg));
    secp256k1_scalar_set_int(&one, 1);
    secp256k1_scalar_add(&msg, &msg, &one);
    CHECK(!secp256k1_gamma_sig_verify(&ctx->ecmult_ctx, &sigr, &sigs, &pub, &msg));
}
...
void test_gamma_aggregate_sign_verify(void) {
    ...
    secp256k1_gej *pubj =  (secp256k1_gej*) malloc (AgSize*sizeof(secp256k1_gej));
    secp256k1_ge *pub =  (secp256k1_ge*) malloc (AgSize*sizeof(secp256k1_ge));
    secp256k1_ge *outpub = (secp256k1_ge*) malloc (AgSize*sizeof(secp256k1_ge));
    secp256k1_ge *A = (secp256k1_ge*) malloc (AgSize*sizeof(secp256k1_ge));
    secp256k1_scalar one;
    secp256k1_scalar *msg =  (secp256k1_scalar*) malloc (AgSize*sizeof(secp256k1_scalar));
    secp256k1_scalar *outmsg =  (secp256k1_scalar*) malloc (AgSize*sizeof(secp256k1_scalar));
    secp256k1_scalar *key =  (secp256k1_scalar*) malloc (AgSize*sizeof(secp256k1_scalar));
    secp256k1_scalar *sigd =  (secp256k1_scalar*) malloc (AgSize*sizeof(secp256k1_scalar));
    secp256k1_scalar *sigz =  (secp256k1_scalar*) malloc (AgSize*sizeof(secp256k1_scalar));
    ...
    for(i = 0; i < AgSize; i++)
      {
          ...
          random_scalar_generation(msg+i);
          random_scalar_generation(key+i);
          secp256k1_ecmult_gen(&ctx->ecmult_gen_ctx,  pubj+i, key+i);
          secp256k1_ge_set_gej( pub+i, pubj+i);
          getrec = secp256k1_rand_bits(1);
          random_gamma_sign( sigd+i,  sigz+i,  key+i,  msg+i, getrec?&recid:NULL);
          if (getrec) {
              CHECK(recid >= 0 && recid < 4);
          }
      }
    ...
    secp256k1_gamma_Agg(&ctx->ecmult_ctx, pub, msg, sigd, sigz, AgSize, outpub, outmsg, A, &outSize  ,&sumZ);

     /*printGe(outpub, outSize);*/

     printf("sadfsd  AgSize =  %d  OutSize =  %d  \n  ",AgSize, outSize);


      CHECK(secp256k1_gamma_Agg_verify( ctx , &ctx->ecmult_ctx, outpub, outmsg, A, outSize, &sumZ));
         secp256k1_scalar_set_int(&one, 1);
     secp256k1_scalar_add( outmsg, outmsg, &one);
      CHECK(!secp256k1_gamma_Agg_verify( ctx , &ctx->ecmult_ctx, outpub, outmsg, A, outSize, &sumZ));
...
void run_gamma_sign_verify(void) {
    int i;
    for (i = 0; i < 10*count; i++) {

    	/*printf("i = %d", i );*/
        test_gamma_sign_verify();
    }

}


void run_gamma_sign_verify_speed(void) {
    int i;
    int num = 1000;

        secp256k1_gej *pubj =  (secp256k1_gej*) malloc (num*sizeof(secp256k1_gej));
        secp256k1_ge *pub =  (secp256k1_ge*) malloc (num*sizeof(secp256k1_ge));
        secp256k1_scalar *msg =  (secp256k1_scalar*) malloc (num*sizeof(secp256k1_scalar));
        secp256k1_scalar *key =  (secp256k1_scalar*) malloc (num*sizeof(secp256k1_scalar));
    ...
    for (i = 0; i < num; i++) {
        random_scalar_order_test(msg+i);
    ...
    print_results("For function secp256k1_gamma_sig_sign: ", t, num);
    ...
    for (i = 0; i < num; i++) {
        	...
        	t[i] = cpucycles();
        	  ...
        	  secp256k1_gamma_sig_verify(&ctx->ecmult_ctx,  sigd+i,  sigz+i,  pub+i,  msg+i)   ;

        }

     print_results("For function secp256k1_gamma_sig_verify: ", t, num);
...
void run_gamma_aggregate_sign_verify(void) {
    ...
    for (i = 0; i < 10; i++) {
    	 ...
    	 printf(" test_gamma_aggregate_sign_verify  count  = %d   \n", i );
        test_gamma_aggregate_sign_verify();
    }
}
...
void run_gamma_aggregate_sign_verify_speed(void) {
	...
	int num = 5;
	 ...
	 unsigned long long t[num];
		...
		int AgSize =100;
		int *outSize=   (int*) malloc (num*sizeof(int));

	    secp256k1_gej *pubj =  (secp256k1_gej*) malloc (num*AgSize*sizeof(secp256k1_gej));
	    secp256k1_ge *pub =  (secp256k1_ge*) malloc (num*AgSize*sizeof(secp256k1_ge));
	    secp256k1_ge *outpub = (secp256k1_ge*) malloc (num*AgSize*sizeof(secp256k1_ge));
	    secp256k1_ge *A = (secp256k1_ge*) malloc (num*AgSize*sizeof(secp256k1_ge));

	    secp256k1_scalar *msg =  (secp256k1_scalar*) malloc (num*AgSize*sizeof(secp256k1_scalar));
	    secp256k1_scalar *outmsg =  (secp256k1_scalar*) malloc (num*AgSize*sizeof(secp256k1_scalar));
	    secp256k1_scalar *key =  (secp256k1_scalar*) malloc (num*AgSize*sizeof(secp256k1_scalar));
	    secp256k1_scalar *sigd =  (secp256k1_scalar*) malloc (num*AgSize*sizeof(secp256k1_scalar));
	    secp256k1_scalar *sigz =  (secp256k1_scalar*) malloc (num*AgSize*sizeof(secp256k1_scalar));
	    secp256k1_scalar  *sumZ =  (secp256k1_scalar*) malloc (num*sizeof(secp256k1_scalar));
	    int recid;
	    int getrec;
	    int i;


	    for(i = 0; i < num*AgSize; i++)
	      {
	        ...
	        for (i = 0; i < num; i++) {
	        	t[i] = cpucycles();
	    	    secp256k1_gamma_Agg(&ctx->ecmult_ctx, pub+i*AgSize, msg+i*AgSize, sigd+i*AgSize, sigz+i*AgSize,
	    	    		AgSize, outpub+i*AgSize, outmsg+i*AgSize, A+i*AgSize,  outSize+i  ,sumZ+i);
	        }


	        print_results("For function run_gamma_aggregate_sign_verify: ", t, num);

	    for (i = 0; i < num; i++) {
	   	        	t[i] = cpucycles();
	   	         CHECK(secp256k1_gamma_Agg_verify( ctx , &ctx->ecmult_ctx, outpub+i*AgSize, outmsg+i*AgSize, A+i*AgSize,
	   	        		 *(outSize+i),  sumZ+i));
	   	        }
   ...
   run_ecdh_tests(); */
    ...
    int i = 0;
       for ( i = 0;i< 10; i++)
       {
           ...
           randombytes(testbuf, 32);
           printChar(testbuf, 32);
       }

*/
/*

    printf("Start run_gamma_sign_verify\n");

     run_gamma_sign_verify();

    printf("Start run_gamma_Aggregate_sign_verify\n");

      run_gamma_aggregate_sign_verify();

*/
    printf("Finish gammaSig Aggregate verify\n");

    run_gamma_sign_verify_speed();
    run_gamma_aggregate_sign_verify_speed();
